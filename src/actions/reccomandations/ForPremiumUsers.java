...
public class ForPremiumUsers extends Reccommendation {
        ...
        for (User user : getDatabase().getUsers()) {
             if (user.getUsername().compareTo(action.getUsername()) == 0) {
                 switch (action.getType()) {
                     case Constants.POPULAR -> getPopularVideos(user);
                     case Constants.FAVORITE -> getFavoriteVideos(user);
                     case Constants.SEARCH -> getVideoBySearch(user, action);
                     default -> new ForPremiumUsers(getDatabase());
                    }
                }
            }
    ...
    public void getFavoriteVideos(final User user) {
        if (user.getSubscriptionType().compareTo(Constants.PREMIUM) != 0) {
    ...
    public void getPopularVideos(final User user) {
        if (user.getSubscriptionType().compareTo(Constants.PREMIUM) != 0) {
